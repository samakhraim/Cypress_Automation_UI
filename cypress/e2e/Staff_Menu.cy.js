...
describe('Staff Menu goals', () => {
  ...
  it('The staff can edit a staff first name', () => {
    ...
    cy.get('tr[class="child"] div[class="row m-0 p-0"] div:nth-child(1) a').contains('Edit').should('exist').click();
    // Clear the first name input and enter a new value
    cy.get('tr[id="edit_staff_Aya"] input[name="first_name"]').should('exist').clear().type('hiba');
  ...
  it('Verify that staff can add a last name to a staff', () => {
    ...
    cy.get('tr[id="edit_staff_Aya"] input[name="last_name"]').should('exist').clear().type('soud');
    // Click the "Save" button to save the changes
    cy.get('.btn.btn-primary[form="form_staff_Aya"]').contains('Save').click({ force: true });
  });
  ...
  it('Verify that staff can clear the last name of a staff', () => {
    ...
    cy.get('tr[id="edit_staff_Aya"] input[name="last_name"]').should('exist').clear();
    // Click the "Save" button to save the changes
    cy.get('.btn.btn-primary[form="form_staff_Aya"]').contains('Save').click({ force: true });
  });
